...
data_cheby = run_or_load(datadir("benchmark_sparse_mkl_cheby.csv"); load=load_csv, force=false) do
...
data_sparse = load_csv(datadir("benchmark_sparse_cheby.csv"))

for group in groupby(data_sparse, :N)
    N = group.N[1]
    fig = plot(
