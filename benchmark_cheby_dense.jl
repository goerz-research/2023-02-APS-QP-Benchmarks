...
benchmark_cheby_with_qdyn(
    Ψ₀, H, tlist;
    rf=datadir("cheby_dense_sanity"),
    cheby_coeffs_limit
)

Ψ_out_qdyn = pyconvert(Vector{ComplexF64},
    qdyn.io.read_psi_amplitudes(
        datadir("cheby_dense_sanity", "final_state.dat.1"),
        N;
        normalize=false
    )
);
...
data_cheby = run_or_load(datadir("benchmark_dense_cheby.csv"); load=load_csv, force=false) do
    run_propagation_benchmark(;
        ...
        callback=(args...; N, precision, kwargs...) -> begin
            ...
            rf = datadir("cheby_dense", rf_name)
